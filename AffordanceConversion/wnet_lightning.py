...
class WnetLightningSystem(pl.LightningModule):
    """
    Wnet Semantic Segmentation + Auto Encoder models training with pytorch lightning
    """
    ...
    def __init__(self, hparams):
        ...
        :param hparams:
        """
        super(WnetLightningSystem, self).__init__()
        self.hparams = hparams
        self.dataset = None
        if self.hparams.solid_only:
            self.encoder_net = UNetAuto(num_in_channels=3, num_out_channels=1, max_features=512)
            self.decoder_net = UNetAuto(num_in_channels=1, num_out_channels=3, max_features=512)
        else:
            self.encoder_net = UNetAuto(num_in_channels=3, num_out_channels=len(AFFORDANCES), max_features=512)
            self.decoder_net = UNetAuto(num_in_channels=len(AFFORDANCES), num_out_channels=3, max_features=512)
    ...
    def forward(self, x):
        ...
        :return:
        """
        segmentations = self.encoder_net.forward(x)
        reconstructions = self.decoder_net.forward(segmentations)
        return segmentations, reconstructions

    # ---------------------
    # TRAINING
    # ---------------------
    def loss(self, images, targets, segmentations, reconstructions):
        """
        ...
        bce_loss = (F.binary_cross_entropy_with_logits(segmentations, targets) * self.loss_factors['loss_bce']).mean()
        # soft_cut_loss = (NCutLoss2D()(segmentations, target) * self.loss_factors['loss_soft_cut']).mean()
        reconstr_loss = (F.mse_loss(reconstructions, images) * self.loss_factors['loss_reconstruction']).mean()
        # smooth_loss = (OpeningLoss2D()(segmentations) * self.loss_factors['loss_smoothing']).mean()
        # loss_total = bce_loss + soft_cut_loss + reconstr_loss + smooth_loss
        loss_total = bce_loss + reconstr_loss
        return loss_total, {
            "loss_total": loss_total,
            "loss_bce": bce_loss,
            # "loss_soft_cut": soft_cut_loss,
            "loss_reconstruction": reconstr_loss, 
            # "loss_smoothing": smooth_loss
        }
    ...
    def training_step(self, batch, batch_idx):
        ...
        segmentations, reconstructions = self.forward(images)
        # calculate loss
        loss_val, loss_dict = self.loss(images, targets, segmentations, reconstructions)
        return {
            'loss': loss_val,
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # calculate loss
        loss_val, loss_dict = self.loss(images, targets, segmentations, reconstructions)
        if batch_idx == 0 and self.trainer.root_gpu is not None and self.trainer.root_gpu == reconstructions.get_device():
            self.log_validation_images(images, targets, segmentations, reconstructions)
        return {
            'val_loss': loss_val,
            'progress_bar': loss_dict,
            'log': loss_dict
    ...
    @rank_zero_only
    def log_validation_images(self, inputs, targets, segmentations, reconstructions):
        # img_grid = make_grid(torch.cat([viz_inputs, pred_solid, target_solid, pred_danger, target_danger]), nrow=len(viz_idxs), padding=20)
        # print(inputs.shape, segmentations.shape, reconstructions.shape, targets.shape)
        viz_inputs = TT.img_norm(inputs)
        viz_reconstructions = TT.img_norm(reconstructions)
        
        if self.hparams.solid_only:
            viz_targets = TT.img_norm(targets, range=(0.0,1.0))
            viz_targets = torch.cat((viz_targets, viz_targets, viz_targets), dim=1)

            viz_segmentations = TT.img_norm(segmentations, range=(0.0,1.0))
            viz_segmentations = torch.cat((viz_segmentations, viz_segmentations, viz_segmentations), dim=1)
        else:
            viz_targets, viz_segmentations = get_normed_map('solid', targets, segmentations)
    
        # pri /nt(viz_inputs.shape, viz_segmentations.shape, viz_targets.shape)
        # print(viz_inputs.shape, viz_segmentations.shape, viz_targets.shape)
        
        img_grid = make_grid(torch.cat([viz_inputs, viz_segmentations, viz_reconstructions, viz_targets]), nrow=inputs.shape[0], padding=20)
...
def main(args):
    ...
    # init module
    model = WnetLightningSystem(args)
